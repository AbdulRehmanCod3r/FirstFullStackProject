...
const EditTerritory: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchTerritory = async () => {
      try {
        ...
        console.error(err);
        alert("Failed to load territory.");
      } finally {
        setLoading(false);
      }
    };
  ...
  // Handle Input Change with LENGTH LIMITS
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;

    // Length validation
    if (name === "territoryDescription" && value.length > 50) return;
    if (name === "regionID" && value.length > 5) return;

    setTerritory({ ...territory, [name]: value });
  };
